...
This document explains the principles of how the Slang Reflection APIs are designed.
For information on how to use the Slang Reflection API functions, please refer to the [Slang User's Guide](https://shader-slang.com/slang/user-guide/reflection.html).

## Goals and Difficulties of Slang Reflection API

Slang provides the "Slang Reflection API" that works for multiple targets. This allows Slang users to write more cross-platform applications that unify similar API functionalities for different APIs. However, since Slang supports multiple targets, there are difficulties in designing a reflection API that works for all of them.
...
When Slang provides advanced language features that a target doesn't support directly, Slang has to "de-sugar" them, and this process must be deterministically well-defined by the Slang Reflection APIs.
...
The Slang Reflection API is based on the input program, and this is the only way to provide consistent binding across multiple platforms.
...
1. When the resource type is a texture, it is bound to a register starting with the letter `t`.
2. When the resource type is a sampler, it is bound to a register starting with the letter `s`.
3. When the resource type is an Unordered Access View (UAV), it is bound to a register starting with the letter `u`.
4. When the resource type is a constant buffer, multiple values are stored in a buffer, and it is bound to a register starting with the letter `b`.
...
```glsl
// Vulkan GLSL Compute Shader Example with Descriptor Sets
...
1. When the resource type is a texture, the parameter is annotated with `[[texture(X)]]`, where `X` is a slot index.
2. When the resource type is a sampler, the parameter is annotated with `[[sampler(X)]]`, where `X` is a slot index.
3. When the resource type is a constant buffer, multiple values can be stored in a buffer, and it is annotated with `[[buffer(X)]]`, where `X` is a slot index.

> **TODO:** Metal has argument buffers that behave like descriptor sets in Vulkan, capable of holding different types of parameter bindings and can be populated beforehand.

> **TODO:** It is unclear if Metal supports "array of descriptors" like HLSL does. When tried, I am getting compile errors. Maybe Shader-playground is using an older version of the Metal compiler?
...
For every variable, Slang has the following concepts and relationships:
...
When the variable is a `struct` type, Slang has the following concepts and relationships:
...
The variable `myOldTexture` uses two `ParameterCategory`s: `ShaderResource` and `SamplerState`. When a type has more than one `ParameterCategory`, its `ParameterCategory` is `Mixed`. You will need to iterate for each category as follows:
...
Note that the `category` in this example plays an important role when querying with `getOffset()`. `myOldTexture` has two offset values: one as `ShaderResource` and another as `SamplerState`.
...
Once you have the offset information, you will need to know the size information because for some types, it may occupy more than one binding slot. For that reason, you need to first query what "kind" of parameter it is. It could be `Scalar`, `Matrix`, `Array`, and so on.
...
`arrayElementStride` in the example illustrates that the size of an array element is not always the same as the distance between each element in an array. `getElementStride` provides the information of how much stride is used in the array.
